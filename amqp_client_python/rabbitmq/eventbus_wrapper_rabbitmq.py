...
class EventbusWrapperRabbitMQ:
    def __init__(
        ...
        self.async_eventbus = AsyncEventbusRabbitMQ(
            config,
            self.loop,
    ...
    def rpc_client(
        ...
        return asyncio.run_coroutine_threadsafe(
            self.async_eventbus.rpc_client(
    ...
    async def async_rpc_client(
        ...
        await self.async_eventbus.rpc_client(
            exchange, routing_key, body, content_type, timeout
        )
    ...
    async def async_publish(
        ...
        await self.async_eventbus.publish(
            event,
            routing_key,
            body,
            content_type,
    ...
    def provide_resource(
        ...
        return asyncio.run_coroutine_threadsafe(
            self.async_eventbus.provide_resource(name, callback, response_timeout),
            self.loop,
        )
